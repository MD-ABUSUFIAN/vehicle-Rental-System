...
const createBooking = async (payload: Record<string, unknown>) => {
  ...
  const searchVehicle = await pool.query(`SELECT* FROM vehicles WHERE id=$1`, [
    vehicle_id,
  ]);
  ...
  const result = await pool.query(
    `INSERT INTO booking(customer_id,vehicle_id,rent_start_date,rent_end_date,total_price,status) VALUES ($1,$2,$3,$4,$5,$6) RETURNING*`,
    [
      customer_id,
      vehicle_id,
      rent_start_date,
